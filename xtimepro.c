...
struct xtimepro_config *xtimepro_default_config ()
{
...
struct xtimepro_stat *xtimepro_read_stat (char *filename, struct xtimepro_stat *stat)
{
  FILE *stat_fp = NULL;
  ...
  if ( (stat_fp = fopen(filename, "r")) == NULL) {
    ...
    fscanf(stat_fp, "%d %d %d", &stat->prev_time, &stat->prev_idle, &stat->total_idle);
...
int xtimepro_write_stat(char *filename, int last_time, int this_idle , int total_idle )
{
  FILE *stat_fp = NULL;
...
int parse_option( struct xtimepro_config *config , int argc , char *argv[])
{
  int opt;
  int hour=0,min=0;
  ...
  while( (opt = getopt(argc, argv, "c:o:s:id" )) != -1 ){
    ...
    case 'i':
    default:
      fprintf(stderr, "usage: %s [-c time] [-o output_file] [-s status_file] [-d] \n", argv[0]);
      fprintf(stderr, "Options\n");
      fprintf(stderr, " -c time    : time period for attendance at day (default 00:00)\n");
...
int main(int argc ,char *argv[]) {
  ...
  if (  this_idle > stat->prev_idle ) { // continues to be idle 
    ...
    if( stat->prev_time == -1 ) {  // this is first event since checktime.
